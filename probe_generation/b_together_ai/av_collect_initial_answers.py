...
def main(
    *_, 
    ...
    try:
        ...
        if truth_status['status'] != 'COMPLETED' or lie_status['status'] != 'COMPLETED':
        ...
        if not success:
            print(f"ERROR downloading truth batch results: {error}")
            sys.exit(1)
        ...
        if not success:
            print(f"ERROR downloading lie batch results: {error}")
            sys.exit(1)
